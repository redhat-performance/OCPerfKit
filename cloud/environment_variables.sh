...
# Put only the id not full ( i.e. /dev/disk/by-id/{{ lso_disk_id }}, default worker-2
export LSO_DISK_ID=''
export LSO_NODE=$(oc get nodes -l node-role.kubernetes.io/worker -o jsonpath='{.items[2].metadata.name}')
